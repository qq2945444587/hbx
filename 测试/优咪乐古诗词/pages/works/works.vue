<template>
	<view class="container">
		...
		<view class="s-list-box" :style="{ paddingTop: top + 'px' }">
			<s-list-view :title="'总收录'+(options.quote_status==1?'名句':'古诗词')+'（'+total+'）'">
...
<script>
	...
	import { getWorkList } from '../../services/index/api.js';
	...
	export default {
		...
		onLoad: function(options) {
			if(options.quote_status == 1){
				uni.setNavigationBarTitle({title: '名句'})
			}
		...
		methods: {
			...
			async getList(){
				...
				if(!this.options.textbook_id && !this.options.dynasty_id && !this.options.solar_term_id){
				...
				if(res.status === 20000){
					if(this.key){
						for(let i = 0;i<res.data.list.length;i++){
							res.data.list[i].title = res.data.list[i].title.replace(this.key, '<span style="color:#ff6b00;font-weight: bold;">'+this.key+'</span>')
						}
					}
					if(!this.list.length){
			...
			async getLibraryData(){
				...
				if(res.status == 20000){
					...
					for(let i = 0;i<res.data[0].children.length;i++){
						if(this.options.textbook_id && res.data[0].children[i].id == this.options.textbook_id){
							res.data[0].children[i].selected = true;
						}else{
					...
					for(let i = 0;i<res.data[1].children.length;i++){
						if(this.options.textbook_id && res.data[1].children[i].id == this.options.textbook_id){
							res.data[1].children[i].selected = true;
						}else{
					...
					for(let i = 0;i<res.data[2].children.length;i++){
						if(this.options.textbook_id && res.data[2].children[i].id == this.options.textbook_id){
							res.data[2].children[i].selected = true;
						}else{
					...
					for(let i = 0;i<res.data[8].children.length;i++){
						if(this.options.dynasty_id && res.data[8].children[i].id == this.options.dynasty_id){
							res.data[8].children[i].selected = true;
						}else{
					...
					for(let i = 0;i<res.data[15].children.length;i++){
						if(this.options.solar_term_id && res.data[15].children[i].id == this.options.solar_term_id){
							res.data[15].children[i].selected = true;
						}else{
		...
		onShareAppMessage(){
			let path = "/pages/works/works"
			let title = "古诗词"
			if(this.options.quote_status == 1){
				path = path + "?quote_status=1"
				title = '古诗词名句'
			}
