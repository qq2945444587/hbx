<template>
	<view class="s-skeleton-cmomon s-skeleton-box"
		:style="{width: winWidth+'px', height:winHeight+'px', backgroundColor:backgroundColor}">
		<view class="s-skeleton-cmomon" :class="{'s-skeleton__active':active}" v-for="(item,index) in skeletonElements" :key="index"
		...
		<view class="s-loading" :class="[getLoadingType(loadingType)]" v-if="isLoading"></view>
	</view>
</template>
...
<script>
	export default {
		...
		props: {
			...
			//默认所有，根据页面情况进行传值
			//页面对应元素class为：s-skeleton-rect，s-skeleton-circular，s-skeleton-fillet
...
<style scoped>
	...
	.s-loading {
		...
		border-radius: 50%;
		border: 2px solid;
		animation: s-rotate 0.7s linear infinite;
		position: fixed;
		z-index: 999999;
		left: 50%;
		top: 50%;
	...
	.s-skeleton__active {
		...
		animation: s-active 1.4s ease infinite;
